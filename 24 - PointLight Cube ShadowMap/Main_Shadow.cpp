...
glm::vec3 pointLightPos = glm::vec3(-2.5f, 4.0f, 4.2f);
...
void RenderScene(Shader shader)
{
	...
	shader.SetVec3("pointLight.diffuse", glm::vec3(0.6f));
	shader.SetVec3("pointLight.specular", glm::vec3(0.8f));
	shader.SetVec3("pointLight.position", glm::vec3(pointLightPos));
	shader.SetFloat("pointLight.constant", 1.0f);
	shader.SetFloat("pointLight.diffuse", 0.09f);
...
int main()
{
	...
	// 点光源空间变换矩阵
	float pointShadowNear = 1, pointShadowFar = 25;
	glm::mat4 pointShadowProj = glm::perspective(glm::radians(90.0f), (float)shadowWidth / (float)shadowHeight, pointShadowNear, pointShadowFar);
	std::vector<glm::mat4> pointShadowView;
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(-1.0, 0.0, 0.0), glm::vec3(0.0, -1.0, 0.0)));
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(0.0, 1.0, 0.0), glm::vec3(0.0, 0.0, 1.0)));
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(0.0, -1.0, 0.0), glm::vec3(0.0, 0.0, -1.0)));
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(0.0, 0.0, 1.0), glm::vec3(0.0, -1.0, 0.0)));
	pointShadowView.push_back(pointShadowProj * glm::lookAt(pointLightPos, pointLightPos + glm::vec3(0.0, 0.0, -1.0), glm::vec3(0.0, -1.0, 0.0)));
	...
	Shader pointShadowShader("Shader/PointShadowMapVertex.glsl", "Shader/PointShadowMapFragment.glsl", "Shader/PointShadowMapGeometry.glsl");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 6; ++i)
		{
			pointShadowShader.SetMat4("shadowMatrices[" + std::to_string(i) + "]", pointShadowView[i]);
		}
		pointShadowShader.SetFloat("farPlane", pointShadowFar);
		pointShadowShader.SetVec3("lightPos", pointLightPos);
		glBindFramebuffer(GL_FRAMEBUFFER, depthCubeMapFBO);
		glClear(GL_DEPTH_BUFFER_BIT);
...

		{
			...
			planeShader.SetInt("pointShadowMap", 1);
			planeShader.SetFloat("farPlane", pointShadowFar);
