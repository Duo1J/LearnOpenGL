...
vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 viewDir, vec3 fragPos)
{
    ...
    // 衰减
    float d = length(normalize(fs_in.TBN * light.position) - fs_in.FragPos);
    float attenuation = 1 / (light.constant + light.linear * d + light.quadratic * d * d);

    // 环境光
    vec3 ambient = light.ambient * vec3(texture(material.tex_diffuse1, fs_in.TexCoords)) * attenuation * intensity;
    // 漫反射
    float diff = max(dot(normal, lightDir), 0);
    vec3 diffuse = light.diffuse * diff * vec3(texture(material.tex_diffuse1, fs_in.TexCoords)) * attenuation * intensity;
    // 镜面反射
    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(reflectDir, viewDir), 0), material.shininess);
    vec3 specular = light.specular * spec * vec3(texture(material.tex_specular1, fs_in.TexCoords)) * attenuation * intensity;
...
void main()
{
    ...
    // 聚光灯
    // result += CalcSpotLight(spotLight, normal, viewDir, fs_in.TangentFragPos);
