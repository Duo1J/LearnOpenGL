...
// MS纹理转普通纹理
unsigned int screenFrameBufferTex;
...
void OnSetFrameBufferSize(GLFWwindow* window, int width, int height)
{
	...
	{
		// MSAA
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, framebufferTex);
		glTexImage2D(GL_TEXTURE_2D_MULTISAMPLE, 0, GL_RGB, screenWidth, screenHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	...
	{
		// MSAA
		glRenderbufferStorageMultisample(GL_RENDERBUFFER, msaaSamples, GL_DEPTH24_STENCIL8, screenWidth, screenHeight);
	}
	...
	if (enableMSAA)
	{
		glBindTexture(GL_TEXTURE_2D, screenFrameBufferTex);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, screenWidth, screenHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glBindTexture(GL_TEXTURE_2D, 0);
...
int main()
{
	...
	else
	{
		// MSAA
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, framebufferTex);
		glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, msaaSamples, GL_RGB, screenWidth, screenHeight, GL_TRUE);
		glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D_MULTISAMPLE, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, 0);
	...
	glGenTextures(1, &screenFrameBufferTex);
	glBindTexture(GL_TEXTURE_2D, screenFrameBufferTex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, screenWidth, screenHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glBindTexture(GL_TEXTURE_2D, 0);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, screenFrameBufferTex, 0);
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			...
			offset.y = j * 5;
			offset.z = 0;
			instanceOffsets[instanceOffsetsIdx++] = offset;
		}
	}
	...
	for (auto it = model.meshes.begin(); it != model.meshes.end(); ++it)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 9, &instanceOffsets[0], GL_STATIC_DRAW);
